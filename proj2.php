...
$data="geom from (select gid, geom, county, name, (SELECT SUM(cases) FROM covid cov WHERE cov.county = name and cov.state = 'New York') as cases, (SELECT SUM(deaths) FROM covid cov WHERE cov.county = name and cov.state = 'New York') as deaths from county) as foo using unique gid using SRID=2263";
